 ...
 */
// Derive the cache-bust version from this script's own ?v= so a single bump in
// index.html (store.js?v=N) propagates to every lazy-loaded module automatically.
const ASSET_VERSION = (() => {
    try { return new URL(document.currentScript.src).searchParams.get('v') || '0'; }
    catch (e) { return '0'; }
})();
...
function store() {
    ...
    return {
        ...
        async loadModule(name) {
            ...
            try {
                const module = await import(`/api-test/store/modules/${name}.js?v=${ASSET_VERSION}`);
        ...
        /** Parse numeric ID from API Platform IRI (e.g. "/api/rest/v2/categories/4" → 4) */
        _parseId(iri) {
        ...
        async subscribeNewsletter() {
            ...
            this.error = null;
            try {
                const res = await fetch('/api/rest/v2/newsletter/subscribe', {
                    method: 'POST',
        ...
        async loadFooterPages() {
            ...
            if (this.useGraphQL && this._gqlQueries) {
                try {
                    ...
                    this.footerPages = this._gqlNodes(data, 'cmsPagesCmsPages');
                } catch (e) {
        ...
        async loadBlogPosts() {
            ...
            if (this.useGraphQL && this._gqlQueries) {
                try {
                    ...
                    this.blogPosts = this._gqlNodes(data, 'blogPostsBlogPosts');
                } catch (e) {
        ...
        async loadCmsPage(pageId) {
            ...
            if (this.useGraphQL && this._gqlQueries) {
                try {
                    const data = await this.gql(this._gqlQueries.CMS_PAGE_QUERY, { id: '/api/rest/v2/cms-pages/' + pageId });
                    this.cmsPage = data.cmsPageCmsPage;
                    this.view = 'cms_page';
                } catch (e) {
        ...
        async api(endpoint, options = {}, retry = true) {
            ...
            const url = '/api/rest/v2' + endpoint;
            const headers = {
                'Content-Type': 'application/json',
        ...
        async viewBlogPost(urlKeyOrId) {
            ...
            try {
                ...
                if (this.useGraphQL && this._gqlQueries) {
                    if (!isNaN(urlKeyOrId)) {
                        const data = await this.gql(this._gqlQueries.BLOG_POST_QUERY, { id: '/api/rest/v2/blog-posts/' + urlKeyOrId });
                        post = data.blogPost;
                    } else {
        ...
        async performInstantSearch() {
            ...
            if (this.useGraphQL && this._gqlQueries) {
                try {
                    ...
                    this.instantSearchPages = this._gqlNodes(pagesData, 'cmsPagesCmsPages')
