...
export default {
    ...
    selectedDownloadableLinks: [],  // For downloadable products: [linkId, ...]
    ...
    async viewProduct(id) {
        ...
        try {
            ...
            if (this.useGraphQL && this._gqlQueries) {
                const data = await this.gql(this._gqlQueries.PRODUCT_QUERY, { id: '/api/rest/v2/products/' + id });
    ...
    allOptionsSelected() {
        ...
        // Downloadable sold separately: require at least one link
        if (this.currentProduct?.type === 'downloadable'
            && this.currentProduct.linksPurchasedSeparately
            && this.currentProduct.downloadableLinks?.length
            && this.selectedDownloadableLinks.length === 0) {
            return false;
        }
    ...
    autoSelectSingleOptions() {
        ...
        // Downloadable: pre-select all links by default
        if (this.currentProduct?.type === 'downloadable' && this.currentProduct.downloadableLinks?.length) {
            this.selectedDownloadableLinks = this.currentProduct.downloadableLinks.map(l => l.id);
        }
    },

    // Downloadable link IDs to send to the cart. When links are sold together
    // (not purchasedSeparately) all links are always included.
    getCartDownloadableLinks() {
        const cp = this.currentProduct;
        if (cp?.type !== 'downloadable' || !cp.downloadableLinks?.length) return [];
        if (cp.linksPurchasedSeparately) return [...this.selectedDownloadableLinks];
        return cp.downloadableLinks.map(l => l.id);
    }
};
