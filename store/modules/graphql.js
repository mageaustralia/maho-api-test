...
query GetCart($maskedId: String!) {
    getCartByMaskedIdCart(maskedId: $maskedId) {
        ...
        billingAddress { firstname lastname city region postcode countryId telephone }
        shippingAddress { firstname lastname city region postcode countryId telephone }
    }
}`;
...
// ME_QUERY removed - GraphQL me query breaks schema, use REST /customers/me instead
const ME_QUERY = null;

const MY_ADDRESSES_QUERY = `
query MyAddresses {
    myAddressesAddresses {
        edges { node { id firstname lastname company city region postcode countryId telephone isDefaultBilling isDefaultShipping } }
        totalCount
    }
}`;
...
mutation CreateAddress($input: createAddressAddressInput!) {
    createAddressAddress(input: $input) {
        address { id firstname lastname company city region postcode countryId telephone isDefaultBilling isDefaultShipping }
    }
}`;
