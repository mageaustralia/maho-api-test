...
export default {
    ...
    async ensureCart() {
        if (!this.cartId) {
            ...
            this.cartId = data.maskedId || data.id || data.cartId;
            if (data.id) { this.cartNumericId = data.id; localStorage.setItem("cartNumericId", String(data.id)); }
            localStorage.setItem('cartId', this.cartId);
        }
        return this.cartId;
    },
