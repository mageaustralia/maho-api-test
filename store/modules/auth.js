...
export default {
    ...
    async login() {
        ...
        try {
            ...
            const response = await fetch('/api/rest/v2/auth/token', {
                method: 'POST',
            ...
            if (this.checkout) {
                ...
                if (this.checkout.shipping) {
                    this.checkout.shipping.firstName = (data.customer.firstName || data.customer.firstname);
                    this.checkout.shipping.lastName = (data.customer.lastName || data.customer.lastname);
                }
            }
            ...
            this.success = 'Welcome back, ' + (data.customer.firstName || data.customer.firstname) + '!';
            this.navigate('home');
        } catch (e) {
    ...
    async loadAccountData() {
        ...
        // GraphQL branch
        if (this.useGraphQL && this._gqlQueries && this._gqlQueries.ME_QUERY) {
            this.loading = true;
            try {
        ...
        this.loading = true;
        try {
            const res = await fetch('/api/rest/v2/customers/me', {
    ...
    async loadOrders(page = 1) {
        ...
        // REST branch
        try {
            const data = await fetch('/api/rest/v2/customers/me/orders?page=' + page + '&pageSize=' + pageSize, {
                headers: { 'Authorization': 'Bearer ' + this.token }
            }).then(r => r.json());
    ...
    async loadOrderInvoices(orderId) {
        ...
        try {
            const data = await fetch('/api/rest/v2/customers/me/orders/' + orderId + '/invoices', {
                headers: { 'Authorization': 'Bearer ' + this.token }
            }).then(r => r.json());
    ...
    async downloadInvoice(orderId, invoiceId) {
        ...
        try {
            const response = await fetch('/api/rest/v2/customers/me/orders/' + orderId + '/invoices/' + invoiceId + '/pdf', {
                headers: { 'Authorization': 'Bearer ' + this.token }
            });
        ...
        try {
            ...
            for (const item of order.items) {
                ...
                try {
                    ...
                    const res = await fetch(`/api/rest/v2/guest-carts/${this.cartId}/items`, {
                        method: 'POST',
                        headers: {
    ...
    printOrder(order) {
        ...
        const formatAddress = (addr) => {
            ...
            return `${esc((addr.firstName || addr.firstname))} ${esc((addr.lastName || addr.lastname))}<br>
                ${esc(street)}<br>
    ...
    async addAddress() {
        ...
        // REST branch
        try {
            const res = await fetch('/api/rest/v2/customers/me/addresses', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
    ...
    async updateAddress() {
        ...
        // REST branch
        try {
            const res = await fetch('/api/rest/v2/customers/me/addresses/' + this.editingAddressId, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': 'Bearer ' + this.token
    ...
    async deleteAddress(addressId) {
        ...
        // REST branch
        try {
            const res = await fetch('/api/rest/v2/customers/me/addresses/' + addressId, {
                method: 'DELETE',
                headers: {
                    'Authorization': 'Bearer ' + this.token
                }
    ...
    async changePassword() {
        ...
        // REST branch
        try {
            const res = await fetch('/api/rest/v2/customers/me/password', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': 'Bearer ' + this.token
    ...
    async forgotPassword() {
        ...
        this.error = null;
        try {
            const res = await fetch('/api/rest/v2/auth/forgot-password', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    async resetPassword() {
        ...
        this.error = null;
        try {
            const res = await fetch('/api/rest/v2/auth/reset-password', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
    ...
    async toggleNewsletter(subscribe) {
        ...
        // REST branch
        try {
            const endpoint = subscribe ? '/api/rest/v2/newsletter/subscribe' : '/api/rest/v2/newsletter/unsubscribe';
